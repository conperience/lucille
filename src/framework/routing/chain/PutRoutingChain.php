<?php declare(strict_types=1);
/**
 * Lucille
 *
 * @author     Andreas Habel <mail@ahabel.de>
 * @copyright  Conperience GmbH, Andreas Habel and contributors
 *
 */

namespace Lucille\Routing;

use Lucille\Command;
use Lucille\Exceptions\RoutingChainConfigurationException;
use Lucille\Request\PutRequest;

/**
 * Class PutRoutingChain
 *
 * @package Lucille\Routing
 */
class PutRoutingChain implements RoutingChain {
    
    /**
     * @var PutRouter
     */
    protected $start;

    /**
     * @var PutRouter
     */
    protected $previous;

    /**
     * @param PutRouter $router PutRouter instance
     * @return void
     */
    public function addRouter(PutRouter $router): void {
        if ($this->start === null) {
            $this->start = $router;
        }
        
        if ($this->previous !== null) {
            $this->previous->setNext($router);
        }
        $this->previous = $router;
    }
    
    /**
     * @param PutRequest $request PutRequest instance
     * @return Command
     * @throws RoutingChainConfigurationException
     */
    public function route(PutRequest $request): Command {
        if ($this->start === null) {
            throw new RoutingChainConfigurationException('No routing target found');
        }
        return $this->start->route($request);
    }
    
}
